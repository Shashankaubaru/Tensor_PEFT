...
def mprod(A, B, M, transpose=True, conjugate=False, inverse=False, device = 'cpu'):
    # default is M is orthogonal
    # modek_args = {'transpose': transpose, 'conjugate': conjugate, 'inverse': inverse}

    A = modek_product(A, M, device=device)
    B = modek_product(B, M, device=device)
    C = facewise(A, B)
    C = modek_product(C, M.T, device=device)
    return C
