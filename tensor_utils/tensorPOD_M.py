import torch
import numpy as np
from tensor_utils.tSVD_M import tSVD_M, construct_projector_M, strideM
...
def tensorPOD_oneside_M(Y, M, k, nx, ny, ns, project=True):
    """
    ...
    W_c = construct_projector_M(tW, M)
    if project:
        return W_c.T @ Y, W_c
    return W_c


def tensorPOD_twoside_M(Y, M1, M2, k1, k2, nx, ny, ns, project=True):
    """
    This function computes the two-sided tensor POD projection of
    snapshot matrix Y and transformation matrix M.
    Y can be either a 2D tensor (nx-x-ny, ns) or a 3D tensor (nx, ns, ny).
    """
    C, W_c = tensorPOD_oneside_M(Y, M1, k1, nx, ny, ns, project=True)
    tC = _matrix_to_tensor(C, nx, k1, ns)
            
    tU = tSVD_M(tC, M2, k2)[0]
    U_c = construct_projector_M(tU, M2)
        
    P = strideM(k1, U_c.size(dim=0))
    U_c = P.T @ U_c
    Q = W_c @ U_c
    if project:
        return U_c.T @ C, Q
